...
if __name__ == '__main__':
    ...
    r = requests.get (url)
    j = r.json()
    #print ("Last price is: %d$" % float(j['lprice']))
    ...
    p.print_text("\nPAYMENT RECEIPT\n")
    p.bold(False)
    p.print_text("      %s/%s/%s  %s:%s:%s" % (d.month, d.day, d.year, d.hour, d.minute, d.second))
    p.justify()
    p.bold()
    p.print_text("\n\n     BTC price")
    p.bold(False)
    p.print_text(":  $%d" % float(j['lprice']))
    p.bold()
    p.print_text("\n     BTC amount")
    p.bold(False)
    p.print_text(": 10,000 Satoshi")
    p.bold()
    p.print_text("\n     USD amount")
    p.bold(False)
    p.print_text(": $0.045\n")

    iqr= Image.open("qr.png")
    dataqr=list(iqr.getdata())
    wqr,hqr= iqr.size
    p.print_bitmap(dataqr, wqr, hqr, True)
    
    p.print_text("18dAhXSeeazgVSmY4Z9XUpN7Njj")
    p.justify("C")
    p.print_text("\n\nPAYMENT COURTESY OF\n")
    p.print_text("EMORY CRYPTO CURRENCY CLUB\n")
    p.justify()
    p.linefeed(3)
